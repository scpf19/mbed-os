...
typedef struct
{
    ...
    /* there are some differences in SCP between JCOP versions */
...
typedef struct
{
    sss_object_t HostEcdsaObj;  // Host ECDSA Private key
    sss_object_t HostEcKeypair; // Host ephemeral ECC key pair
    sss_object_t SeEcPubKey;    // SE ECC public key
    sss_object_t masterSec;      // Host master Secret
...
/* Identifier to be used to connect to the
 * secure sub system */
typedef struct
{
    sss_object_t * pObj;
...
typedef struct _SE_AuthCtx
{
     ...
     * Systems may have some specific ways of authenticating
     *
     *
     */
